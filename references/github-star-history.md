...
用户询问 README 里的 Star History 为什么失效、以后还能不能显示、怎样替换，要求为一个或多个 GitHub 仓库接入长期可维护的星标历史图，或 `readme-delivery.md` 已判定当前任务是现有公开 GitHub 仓库的完整 README 优化时读取本页。最后一种情形是默认交付，不等待用户再次点名 Star History。

Star History 是目标仓库 README 与主页展示的独立优化结果。仅仅调用 Project Steward、要求它自我进化、进行项目综合审计、修改普通项目或发布仓库，都不会触发本页；只有明确的 Star History 结果，或 README 主路径已经判定为完整优化时才适用。本页的提交、推送和工作流派发权限只属于这项展示结果，不能扩张到其它项目改动或 Project Steward 自我进化。

只问原因、现状或复杂度时保持只读：检查 README 当前图片来源、实际 HTTP 结果、仓库可见性、Actions 策略、工作流是否存在和启用、最近已经完成的运行结果以及输出分支，不修改仓库，也不等待进行中的运行。用户明确要求接入、修复或替换，或完整 README 优化的默认交付条件成立时进入实施；除非用户明确退出 Star History 或要求只改本地，完成位置默认到精确提交、推送和工作流派发。派发成功后立即停止并把远端图表生成与 README 消费标为异步未验证；只有用户另行明确要求远端验收时才读取已经完成的运行。目标不是公开 GitHub 仓库或没有现有远端时标为不适用；权限、Actions 或推送失败时标为受阻，不得把失败重新解释为“不需要”。
...
4. 推送成功后手动派发一次调用仓库的工作流，派发成功后立即停止，不运行 `gh run watch` 或其它轮询：

```powershell
gh workflow run star-history.yml --repo OWNER/REPOSITORY --ref DEFAULT_BRANCH
```

5. 用户后来明确要求远端验收时，从该次已经结束的运行核对 `star-history` 分支、两个 SVG、日志星标数、两个 raw 地址和 GitHub README 的明暗主题显示。需要验证 `changed=false` 时另行派发并再次停止，不在同一任务中等待。

普通实施只报告已经写入、已经提交、已经推送和工作流已经派发，不概括成图表已经可用。独立远端验收只有在上述链路全部成立时才报告图表可用；工作流尚未完成、输出分支尚未生成、raw 地址仍返回错误或 README 仍无法显示时，分别报告异步未验证或准确失败层。
...
用户明确要求远端验收时，至少核对已经结束的工作流运行成功、日志报告的星标数等于仓库当前星标数、输出分支同时存在两个 SVG、原始地址返回 SVG 内容，并在 GitHub README 的深浅主题入口确认图像能够显示。再次手动运行且星标未变时，派发后停止；下一次验收再确认生成器报告 `changed=false`，输出分支没有新增提交。
