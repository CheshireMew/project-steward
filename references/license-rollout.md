...
许可证文件写对不代表项目的正式许可证声明已经一致。实施前先列出同一项目中全部声明消费者：根级许可证与范围说明、README 或发布文档摘要、生态项目清单和工作区包清单中的许可证字段、根包元数据、锁文件中的根项目记录，以及包管理器或注册表工具从这些文件解析出的许可证输出。只修改当前项目自己的声明；依赖、vendor 和第三方组件各自的许可证记录保持独立，不能因根项目换证而批量改写。

`apply_license.py` 继续只写获准的许可证治理文件。项目清单和锁文件由各生态的正式配置入口、包管理器或经过授权的源码修改更新，再从实际解析输出核对；不能为了省事扩张许可证脚本的任意文件写入范围。全量预检和验证只有在这些正式消费者对当前项目给出同一范围结论、依赖声明未被误改时才通过。
