...
- 联网、下载、安装、运行、生成、写入、移动、归档、删除、提交、推送和发布分别授权；只有本节随后明确列出的默认完成合同可以合并同一结果内的写入、提交和推送。运行或生成不等于获得项目写入权限；远端检查、部署和监控继续是独立结果，提交或推送不自动授权等待 GitHub Actions。
- 用户明确点名 Project Steward（包括调用 `$project-steward`），并明确要求对现有 Git 项目实施修改、修复、治理、更新或优化时，若当前分支存在已配置 upstream 且未指定早停，默认按获准依赖闭包验证、精确提交并非强制推送当前跟踪分支；推送成功立即停止，不轮询 GitHub Actions、部署或远端验证。只读请求、隐式路由、无 upstream 不适用；创建远端、改变可见性、删除、强推、改写历史、打包、部署和监控不获授权。遇分叉、未知变化、未授权删除、凭据、验证或认证失败时按 `references/repository-publication.md` 停止。
...
- 用户明确要求为现有 GitHub 仓库接入、修复或替换 Star History 时，确认合同默认包含本轮获准依赖闭包的提交、推送和工作流派发；除非用户明确要求只改本地，否则写入验证后不再等待第二次发布授权，派发成功后也不等待工作流完成。该默认不包含创建远端、强制推送、改变可见性、删除、远端监控或无关工作树变化。
