...
- 联网、下载、安装、运行、生成、写入、移动、归档、删除、提交、推送和发布分别授权；只有本节随后明确列出的默认完成合同可以把同一用户结果内的写入、提交、推送和远端验收合并授权。运行或生成不等于获得项目写入权限；默认完成合同之外，提交、推送或发布只授权处理已存在且准确核对的改动，不追溯授权先前写入，也不包含无关工作树变化。
- 用户在请求中明确点名 Project Steward（包括调用 `$project-steward`），并明确要求对现有 Git 项目实施修改、修复、治理、更新或优化时，若当前分支存在已配置 upstream 且用户没有要求只改本地、不要提交、不要推送或指定更早停止位置，本轮实施默认完成到：按获准依赖闭包验证、精确提交、非强制推送当前跟踪分支，再核对远端 HEAD 与关键消费者；本地验证后不再第二次询问是否推送。只读请求、隐式路由、无 upstream、创建远端、改变可见性、删除、强制推送、改写历史、打包、部署和无关工作树变化不由该默认获得授权；按 `references/repository-publication.md` 在分叉、来源不明变化、未获授权删除、凭据、验证或认证失败时停止。
