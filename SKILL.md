...
只点名本 Skill 时据材料选主路径；不足则给请求示例并停止。

每个独立结果行动前建立路由账本：结果、主路径、固定方法、条件专项及证据、动作门槛、消费者、验收和停止位置。固定方法不因动作明显或本轮较早读过而跳过；条件专项凭证据叠加。同一结果增项更新原账本；后续独立结果重新选路、重读固定方法、新建账本；新症状不换旧范围。
