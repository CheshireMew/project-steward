...
- 用户确认方案且未要求只改本地时，当前跟踪远端存在即把整个工作区按实际影响验证：验证已跟踪修改、未跟踪文件和现有删除，使用 `git add -A` 提交并推送；推送成功后停止且不等待远端检查。任何一项不能共同发布时保留完整工作区并停止，不退回选择性提交；不授权创建远端、强推、改可见性、新删除或远端验证。
...
- 来源读到末尾；长文件按稳定区间读取。会话页 `hasMore=false` 只关闭当前返回层；消息为空或被摘要替代时继续到稳定原始记录，否则不得声称完整。记录 Skill 与方法的内容身份；自动续跑、压缩恢复、结项前复核只重读变化文件。变化触及合同、消费者、验证或用户结果时，旧确认失效，重新规划并确认。
