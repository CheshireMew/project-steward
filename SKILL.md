...
只点名本 Skill 时按材料选主路径；材料不足就给出一个直接请求示例并停下。

每个独立结果在项目写入或专项实施前建立路由账本：用户结果、主路径、固定方法、条件专项及触发证据、动作门槛、正式消费者、验收和停止位置。固定方法不得因局部动作明显或本轮较早读过而跳过；条件专项只在适用时叠加。同一结果增项更新原账本；后续独立结果重新选路、重读固定方法、新建账本，新症状不替换旧范围。
...
用户只要求分析时保持只读。明确要求吸收、自我进化、更新、优化或迭代 Project Steward 时，先交付未来行为、职能边界、代表性输出、影响文件和验证方式并停下；用户确认该方案后才修改活动 Skill。确认后未声明只改本地则读 `references/repository-publication.md`；有跟踪远端就把整个工作区按实际影响验证，使用 `git add -A` 将已跟踪修改、未跟踪文件和现有删除一起提交并推送当前分支，推送成功后停止，不等待远端检查。远端、认证、保护规则、分叉、验证失败或任一项不能共同发布时保留整个工作区并停下，不退回选择性提交。
...
修复先前综合审计的全部问题时，原交接账本仍为结项合同。写入前进入 `references/change-prevention.md` 完整预防链；桌面项目涉及外壳、原生进程、界面线程、延迟工作、原生窗口或捕获时，再读 `references/desktop-app-governance.md`；移动或归档再读 `references/repository-directory-governance.md`。末次相关修改后回到 `references/project-audit.md`，按原范围和合同重建账本。无关历史审计不扩张单点缺陷范围。
